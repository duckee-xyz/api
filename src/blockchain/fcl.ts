...
declare module '@onflow/fcl' {
  export namespace InteractionTemplateUtils {
    ...
    function generateDependencyPinAtLatestSealedBlock(opts: Record<string, unknown>): string;

    function generateTemplateId(opts: Record<string, unknown>): string;

    function verifyInteractionTemplateAudit(opts: Record<string, unknown>): string;

    function deriveCadenceByNetwork(opts: Record<string, unknown>): string;
  }

  // CONFIGURATION
  export type Environment = 'local' | 'canarynet' | 'sandboxnet' | 'testnet' | 'mainnet';
